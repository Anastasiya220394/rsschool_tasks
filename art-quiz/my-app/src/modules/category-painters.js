...
async function getData(e) {
  ...
  const authorRandom = data[Math.floor(Math.random() * data.length)].author;
  const authorRandom2 = data[Math.floor(Math.random() * data.length)].author;
  const authorRandom3 = data[Math.floor(Math.random() * data.length)].author;
  ...
  function modal() {
    ...
    if (b <= 7) {
      ...
      document.querySelector('.score-results').textContent = `${'Game over' + ' '}${b}/` + '10';
    ...
    if (b === 0) {
      document.querySelector('.modal-results-cup').src = './assets/svg/game-over-cup.svg';
      document.querySelector('.congr').textContent = 'Сыграем еще?';
      document.querySelector('.score-results').textContent = 'Game over' + ' ' + '0' + '/' + '10';
      document.querySelector('.results-painters__btnHome').textContent = 'Отмена';
      document.querySelector('.results-painters__btnNext').textContent = 'Да';
    } else {
